<x-project-features-layout :project="$project" :features="$allProjectFeatures" :active-feature-id="$feature->id">
    ...
    <form method="POST" action="{{ route('projects.features.update', [$project, $feature]) }}">
        ...
        <div class="mb-4">
            ...
            <x-text-input id="time_allotted" class="block mt-1 w-full" type="number" name="time_allotted" :value="old('time_allotted', $feature->time_allotted)" required min="0" />
        ...
        <div class="mb-4">
            ...
            <textarea id="progress" name="progress" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('progress', $feature->progress) }}</textarea>
